...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt btcutil.Amount) *chainhash.Hash {
		...
		// Next, send amt CTT to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(btcutil.Amount(5 * btcutil.MystikoPerBitcoin))
	...
	txid = genSpend(btcutil.Amount(500 * btcutil.MystikoPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(btcutil.MystikoPerBitcoin, pkScript)
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 CTT.
	expectedBalance := btcutil.Amount(250 * btcutil.MystikoPerBitcoin)
	...
	// The original wallet should now have a balance of 0 CTT as its entire
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := btcutil.Amount(50 * btcutil.MystikoPerBitcoin)
	...
	// The current wallet balance should now be at least 50 CTT less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 CTT) of mature unspendable
	// outputs.
	expectedBalance := btcutil.Amount(numMatureOutputs * 50 * btcutil.MystikoPerBitcoin)
