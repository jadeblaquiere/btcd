...
func TestAmountCompression(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "0 CTT (sometimes used in nulldata)",
			uncompressed: 0,
			compressed:   0,
		},
		{
			name:         "546 Mystiko (current network dust value)",
			uncompressed: 546,
			compressed:   4911,
		},
		{
			name:         "0.00001 CTT (typical transaction fee)",
			uncompressed: 1000,
			compressed:   4,
		},
		{
			name:         "0.0001 CTT (typical transaction fee)",
			uncompressed: 10000,
			compressed:   5,
		},
		{
		...
		{
			name:         "21000000 CTT (max minted coins)",
...
func TestCompressedTxOut(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "pay-to-pubkey uncompressed 1 CTT",
			amount:     100000000,
